...
export default function SidebarSettingsModal({ theme, t, onClose }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const sidebarLayout = items.map((i) => ({ key: i.key, hidden: !!i.hidden }));
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          width: 400, maxWidth: '100%', maxHeight: '85vh', overflowY: 'auto', background: theme.dark ? 'oklch(0.17 0.02 255)' : '#ffffff', border: `1px solid ${theme.border}`,
